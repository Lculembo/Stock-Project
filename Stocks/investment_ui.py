...
def main():
    ...
    with tab1:
            ...
            if 'latest_analysis' in st.session_state and st.session_state.latest_analysis['symbol'] == symbol:
                ...
                with col2_3:
                    st.metric(
                        ...
                        delta=f"Risk Score: {analysis['risk_metrics']['risk_score']:.3f}"
                    )
        ...
        if 'latest_analysis' in st.session_state and st.session_state.latest_analysis['symbol'] == symbol:
            ...
            with gauge_col2:
                fig2 = create_score_gauge(analysis['layer3']['stacked_score'], "Layer 3 Stacked")
                st.plotly_chart(fig2, use_container_width=True)
            
            with gauge_col3:
                fig3 = create_score_gauge(analysis['layer4']['risk_adjusted_score'], "Layer 4 Risk-Adjusted")
                st.plotly_chart(fig3, use_container_width=True)

            with gauge_col4:
                fig4 = create_score_gauge(1 - analysis['risk_metrics']['risk_score'], "Risk Safety (Higher Better)")
            ...
            with col_left:
                ...
                st.metric("Layer 3 Model", analysis['layer3']['model'])
            ...
            with col_right:
                ...
                st.metric("Risk Score", f"{analysis['risk_metrics']['risk_score']:.3f}")

            st.subheader("Risk Layer Details")
            risk_col1, risk_col2, risk_col3, risk_col4 = st.columns(4)
            with risk_col1:
                st.metric("Volatility", f"{analysis['risk_metrics']['volatility']:.1%}")
            with risk_col2:
                st.metric("Max Drawdown", f"{analysis['risk_metrics']['max_drawdown']:.1%}")
            with risk_col3:
                st.metric("Beta vs SPY", f"{analysis['risk_metrics']['beta_to_spy']:.2f}")
            with risk_col4:
                st.metric("VaR (95%)", f"{analysis['risk_metrics']['value_at_risk_95']:.2%}")
    ...
    with tab2:
        ...
        if 'portfolio_analysis' in st.session_state:
            ...
            ranking_mode = st.radio(
                ...
                options=["Layer 4 (Risk-Adjusted)", "Layer 3 (Raw Stacked)"],
                horizontal=True,
                key="portfolio_ranking_mode"
            )
            show_side_by_side = st.checkbox(
                "Show side-by-side Layer 3 vs Layer 4 rankings",
                value=True,
                key="portfolio_side_by_side"
            )
            ...
            for rec in portfolio['recommendations']:
                ...
                portfolio_rows.append({
                    'Symbol': rec['symbol'],
                    'Recommendation': analysis['recommendation'],
                    'Investment Score': float(analysis['investment_score']),
                    'Layer3': float(analysis['layer3']['stacked_score']),
                    'Layer4': float(analysis['layer4']['risk_adjusted_score']),
                    'Risk Score': float(analysis['risk_metrics']['risk_score']),
                    'Position Size': float(analysis['position_size']),
                    'Momentum': float(analysis['momentum']['score']),
                    'Fundamentals': float(analysis['fundamentals']['score']),
                    'Expected Return': float(analysis['fundamentals']['expected_return'])
                })

            df_portfolio = pd.DataFrame(portfolio_rows)

            if len(df_portfolio) > 0:
                df_rank_l3 = df_portfolio.sort_values('Layer3', ascending=False).reset_index(drop=True)
                df_rank_l4 = df_portfolio.sort_values('Layer4', ascending=False).reset_index(drop=True)

                if show_side_by_side:
                    side_col1, side_col2 = st.columns(2)
                    with side_col1:
                        st.caption("Layer 3 ranking (raw stacked score)")
                        st.dataframe(df_rank_l3, use_container_width=True)
                    with side_col2:
                        st.caption("Layer 4 ranking (risk-adjusted score)")
                        st.dataframe(df_rank_l4, use_container_width=True)

                primary_df = df_rank_l4 if ranking_mode == "Layer 4 (Risk-Adjusted)" else df_rank_l3
                st.caption(f"Primary table sorted by: {ranking_mode}")
                st.dataframe(primary_df, use_container_width=True)

                # Portfolio allocation chart
                st.subheader("Portfolio Allocation")
                fig = px.pie(
                    values=primary_df['Position Size'],
                    names=primary_df['Symbol'],
                    title=f"Recommended Position Sizes ({ranking_mode})"
                )
                        ...
                        try:
                            ...
                            if 'error' not in analysis:
                                ...
                                comparison_data.append({
                                    ...
                                    'Layer3 Stacked': a['layer3']['stacked_score'],
                                    'Layer4 Risk-Adj': a['layer4']['risk_adjusted_score'],
                                    'Risk Score': a['risk_metrics']['risk_score'],
    ...
    with tab4:
        ...
        st.markdown("""
        This advanced investment system combines four layers of analysis:
        ...
        **Layer 3: Meta-Learning & Regime Detection**
        ...
        - True stacked model combines momentum + fundamentals + regime features
        - Learns blend behavior from your chosen training symbols

        **Layer 4: Dedicated Risk Layer**
        - Independent risk scoring (volatility, drawdown, beta, VaR)
        - Risk-adjusted final score before recommendation
        - Position sizing scaled by risk and confidence
        """)
